...
with open('setup.config') as search:
    for line in search:
        ...
        if "coords " in line:
            coords=line.replace('coords ','').replace('(','').replace(')','')
...
def currentTimeandDate():
    ...
    return 'Today is '+monthName((now.month))+' '+str(now.day)+' in year '+str(now.year)+' at '+str(now.strftime("%H:%M"))
def joke():
...
def weatherNow(offline):
    if offline==True:
        ...
        ds=http.request("GET",'https://api.darksky.net/forecast/1bdb859538ecbcf26d45997da67be01f/{}'.format(coords)).data
    ...
    return formulateReport(actual(ds),feel(ds),humidity(ds))
